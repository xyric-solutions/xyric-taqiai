/*
 * PakistanLawSite 1981-1990 Chrome paste download/capture launcher.
 *
 ...
 * capture server on 127.0.0.1:8782.
 *
 * Start the local capture server first:
 *   D:\AI legal System\ai-legal-system\scripts\Start PLS 1981-1990 Fast Capture.bat
 */
(function pls1981To1990ChromeLauncher() {
  "use strict";
  ...
  var CONFIG = {
    ...
    sessionProbeMax: 5,
    serverStartHint: 'Run this in PowerShell/CMD, not Chrome: "D:\\AI legal System\\ai-legal-system\\scripts\\Start PLS 1981-1990 Fast Capture.bat"'
  };

  function overlay(message, tone) {
    var box = document.getElementById("codex-pls-1981-1990-launcher-status");
    if (!box) {
      box = document.createElement("div");
      box.id = "codex-pls-1981-1990-launcher-status";
      box.style.cssText = [
        "position:fixed",
        "z-index:2147483647",
        "left:14px",
  ...
  function validateServer(status) {
    ...
    if (!status.total || status.total <= 0) {
      overlay(
        "The 1981-1990 capture server has zero worklist rows.\n"
          + "Run worklist discovery first, then restart the capture server.\n\n"
          + 'Discovery: "D:\\AI legal System\\ai-legal-system\\scripts\\Start PLS 1981-1990 Worklist Discovery.bat"',
        "error"
      );
      return false;
    }
