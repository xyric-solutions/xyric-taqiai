/*
 * PakistanLawSite 1971-1980 Chrome paste launcher.
 *
 ...
 * capture server on 127.0.0.1:8781.
 *
 * Start the local capture server first:
 *   D:\AI legal System\ai-legal-system\scripts\Start PLS 1971-1980 Fast Capture.bat
 */
(function pls1971To1980ChromeLauncher() {
  "use strict";
  ...
  var CONFIG = {
    ...
    sessionProbeMax: 5,
    serverStartHint: 'Run this in PowerShell/CMD, not Chrome: "D:\\AI legal System\\ai-legal-system\\scripts\\Start PLS 1971-1980 Fast Capture.bat"'
  };

  function overlay(message, tone) {
    var box = document.getElementById("codex-pls-1971-1980-launcher-status");
    if (!box) {
      box = document.createElement("div");
      box.id = "codex-pls-1971-1980-launcher-status";
      box.style.cssText = [
        "position:fixed",
        "z-index:2147483647",
        "left:14px",
  ...
  function validateServer(status) {
    ...
    if (CONFIG.expectedTotal && status.total && status.total !== CONFIG.expectedTotal) {
      overlay("Warning: worklist total is " + status.total + ", expected " + CONFIG.expectedTotal + ".", "warn");
    }
    return true;
  }
