...
export default function TemplatePage() {
  ...
  const template = getTemplate("criminal-law", subType);
  ...
  if (!template) {
    return <div className="text-center py-12"><h2 className="text-xl font-bold text-slate-800">Template not found</h2><Button variant="outline" className="mt-4" onClick={() => router.push("/criminal-law")}>Back</Button></div>;
  }

  const handleSubmit = async (formData: Record<string, string>) => {
    setLoading(true); setError(null);
    try {
      const res = await fetch("/api/ai/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ category: "criminal-law", subType, formData, language }) });
      if (!res.ok) throw new Error((await res.json()).error || "Failed");
      const data = await res.json();
      setGeneratedContent(data.html);
      const saved = await saveDocument({ title: template.name, category: "criminal-law", subType, language, content: data.html });
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/criminal-law" className="text-slate-400 hover:text-slate-600"><ArrowLeft className="h-5 w-5" /></Link>
        <div>
      ...
      {loading ? (
        ...
        <Card className="p-4 sm:p-6">
          ...
          <ImageUploadTyping onResult={async (html) => { setGeneratedContent(html); const saved = await saveDocument({ title: template.name, category: "criminal-law", subType, language, content: html }); if (saved) setSavedDocId(saved.id); }} language={language} />
        </Card>
      )}
    </div>
  );
