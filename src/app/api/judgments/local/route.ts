...
export async function GET(req: NextRequest) {
  ...
  if (relatedMode) {
    ...
    const total = await countRelatedJudgments(query, court, year, reportedOnly);
    const hasMore = rel.length > PAGE_SIZE;
    return NextResponse.json({
      results: rel.slice(0, PAGE_SIZE), related: true, hasMore, page, total, totalPages: pagesFor(total), stats: await getJudgmentDbStats(),
    });
  }
  ...
  const total = await countLocalJudgments(query, court, year, reportedOnly);
  const hasMore = found.length > PAGE_SIZE;
  return NextResponse.json({
    results: found.slice(0, PAGE_SIZE), related: false, hasMore, page, total, totalPages: pagesFor(total), stats: await getJudgmentDbStats(),
  });
}
